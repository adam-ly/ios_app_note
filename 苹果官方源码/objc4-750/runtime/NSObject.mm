...
struct SideTable {
    ...
    RefcountMap refcnts;//引用计数 hashmap
    weak_table_t weak_table; //存储对象的弱引用指针,重点
...
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    if (haveOld) {
        oldObj = *location; // location : Address of __weak ptr.  *location == weakobj
        oldTable = &SideTables()[oldObj];//如果weak ptr之前弱引用过一个obj,以Obj为key,获取对应的sideTable
    } else {
        oldTable = nil;
    }
    if (haveNew) {
        newTable = &SideTables()[newObj];//以newObj为key,获取对应的sideTable
    } else {
        newTable = nil;
    }
    ...
    if (haveNew  &&  newObj) {
        ...
        {
            SideTable::unlockTwo<haveOld, haveNew>(oldTable, newTable);
            //如果cls(⚠️不是obj)没有初始化,要先初始化然后继续设置weak
    ...
    if (haveNew) {
        //保存新值 : 调用weak_register_no_lock方法,将weak ptr(location)的地址记录到newObj的weaktable的weak_entry_t中
        newObj = (objc_object *)
...
objc_initWeak(id *location, id newObj)
{
    ...
    // true true no location(Address of __weak ptr) newObjec ptr
