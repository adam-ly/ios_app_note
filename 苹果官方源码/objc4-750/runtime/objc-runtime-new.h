 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            uint32_t newCount = oldCount + addedCount;//更新方法数量
...
struct class_rw_t {
    ...
    method_array_t methods; //存放方法
    property_array_t properties; //存放属性
    protocol_array_t protocols; //存放协议
...
public:
    ...
    void setData(class_rw_t *newData)
    {
         ...
         * 在 x86_64 架构上，MacOS只使用了其中47位来为对象分配地址。
         * 而且由于地址要按字节在内存中按字节对齐，所以掩码的后三位都是 0。
         */
