                   ...
                   bool baseMethods, bool methodsFromBundle)
{
    ...
    for (int i = 0; i < addedCount; i++) {
        method_list_t *mlist = addedLists[i]; // //逐一取出方法数组
        assert(mlist);
...
// ⚠️⚠️⚠️oldest categories first.⚠️⚠️⚠️
static void 
   ...
   #warning 类别入口
    ...
    for (EACH_HEADER) {
        ...
        for (i = 0; i < count; i++) {
            ...
            // Process this category.继续添加类别
            // First, register the category with its target class. //首先,给目标类注册类别
            // Then, rebuild the class's method lists (etc) if  //然后,重建类的方法列表,(如果类被实现的话)
